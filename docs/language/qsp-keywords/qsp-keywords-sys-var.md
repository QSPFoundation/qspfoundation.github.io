...
В первых двух случаях для каждой отдельной сессии таких блоков кода плеер будет создавать собственный массив `args`.

**Однако:**

* в циклах будет использоваться массив `args`, созданный в блоке кода, из которого цикл был вызван.
* в действиях и гиперссылках будет использоваться массив `args`, созданный на локации, на которую был осуществлён переход с помощью  операторов `goto` или `xgoto`.
