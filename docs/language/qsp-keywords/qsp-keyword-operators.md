...
При "суммировании" строковых значений производится [конкатенация](qsp-keyword-operators.md#-конкатенация), то есть "склеивание" строк:
...
Возможно так же проводить инкремент со строковыми значениями, работает как [конкатенация](qsp-keyword-operators.md#-конкатенация):
...
:::tip
Не рекомендуется использовать данную операцию для объединения строк, поскольку строки можно объединять через [`+`](qsp-keyword-operators.md#-сложение):
