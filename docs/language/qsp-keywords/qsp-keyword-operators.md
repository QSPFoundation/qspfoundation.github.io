...
:::note[Допустима, но не рекомендуется, запись:]
...
### Однострочный `IF` с `ELSE`

Однострочная форма записи с использованием альтернативной ветки команд:

```qsp
IF [#выражение]: [команда д1] & [команда д2] & ... ELSE [команда н1] & [команда н2] & ...
```

Здесь снова за ключевым словом `if` следует `[#выражение]`, истинность которого требуется проверить. Если выражение верно, выполняется набор команд `[команда д1]`, `[команда д2]` и т.д., от двоеточия до ключевого слова `else`. В противном случае, если выражение `[#выражение]` неверно, выполняется набор команд `[команда н1]`, `[команда н2]` и т.д., от `else` и до конца строки. Концом конструкции условия считается конец строки.
...
### Однострочный `IF` с `ELSEIF`

Однострочная форма записи с несколькими альтернативными условиями выглядит так:

```qsp
IF [#выражение_1]: {команды 1} ELSEIF [#выражение_2]: {команды 2} ... ELSEIF [#выражение_N]: {команды N} ELSE {команды N+1}
```

Здесь если верно `[#выражение_1]`, выполняются `{команды 1}`, в противном случае проверяется условие `[#выражение_2]`. Если верно оно, выполняются `{команды 2}`, в противном случае проверяется следующее условие для `elseif` и т.д. Если ни одно из условий не верно, выполняются `{команды N+1}` после ключевого слова `else`.
...
### Многострочный `IF` c `ELSE`

Многострочная форма с альтернативной веткой команд в общей записи выглядит так:
...
В многострочной версии `if` допускается использование однострочного варианта `else`:
...
if obj("Апельсин"):
    pl "У Вас есть апельсин."
else pl "У Вас нет апельсина."
end

if len($curtoken) > 0:
    $tokens[curtoken] = $curtoken
    tokens = curtoken + 1
else tokens = curtoken
end
```

Это несколько снижает читаемость кода, поэтому применяйте однострочную форму `else` в многострочной конструкции условия только, если это оправдано.

### Многострочный `IF` c `ELSEIF`

Общая форма записи многострочной конструкции условий с альтернативными взаимноисключающими условиями такая:
